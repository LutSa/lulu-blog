...
view model =
    let
        ...
        renderHtml = if model.state == One || model.state == DiscoverI || model.state == Two ||
                        ...
                        [ renderButton Message.Start (ImagePath.toString Pages.images.img.button.startBut) model.size (Pos 1100 1300 200 350)
                        , renderButton Message.About (ImagePath.toString Pages.images.img.button.aboutBut) model.size (Pos 1100 1300 400 550) ]
                    else if  model.state == End then
                        [ renderButton Next (ImagePath.toString Pages.images.img.button.nextBut) model.size (Pos 1100 1300 400 500) ]
                    else if model.state == Model.About then
                        [ renderButton Back (ImagePath.toString Pages.images.img.button.backBut) model.size (Pos 1100 1300 400 500) ]
                    else
...
renderBlood player = 
    ...
    if player.mood == Normal then
        if player.hp >= 7 && player.hp <10 then
            [ renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_1) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeOne)/2000))]
            ]
        else if player.hp >= 5 && player.hp < 7 then
            [ renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_1) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeTwo)/1750))]
            , renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_2) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeOne)/1750))]
            ]
        else if player.hp >= 3 && player.hp < 5 then
            [ renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_1) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeThree)/1500))]
            , renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_2) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeTwo)/1500))]
            , renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_3) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeThree)/1500))]
            ] 
        else if player.hp > 0 && player.hp < 3 then
            [ renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_1) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeFour)/1250))]
            , renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_2) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeTwo)/1250))]
            , renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_3) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeThree)/1250))]
            , renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_4) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeFour)/1250))]
            ]  
        else
            [ renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_1) (Pos 0 0 0 0) [opacity "0"]
            ]
    else 
        [ renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_1) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeFive)/1000))]
        , renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_2) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeThree)/1000))]
        , renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_3) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeFour)/1000))]
        , renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_4) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeFive)/1000))]
        , renderImage (ImagePath.toString Pages.images.img.effect.bloodFrame_5) (Pos 0 1600 0 800) [opacity (String.fromFloat (abs(player.effecttimeFive)/1000))]
        ]
...
renderbrick1 model pos=
    let
        ...
        text = if model.state == One then
                    ImagePath.toString Pages.images.img.stone.map_1.stone_1
                else if model.state == DiscoverI then
                    if pos.x2-pos.x1 >= pos.y2-pos.y1 then
                        ImagePath.toString Pages.images.img.stone.map_2.stone_1

                    else
                        ImagePath.toString Pages.images.img.stone.map_2.stone_2
                else if model.state == Two then
                    if pos.x2-pos.x1 >= pos.y2-pos.y1 then
                        ImagePath.toString Pages.images.img.stone.map_3.stone_1
                    else if pos.y2-pos.y1 >= 1000 then
                        ImagePath.toString Pages.images.img.stone.map_3.stone_3
                    else
                        ImagePath.toString Pages.images.img.stone.map_3.stone_2
                else if model.state == DiscoverII then
                    if pos.x2-pos.x1 >= pos.y2-pos.y1 then
                        ImagePath.toString Pages.images.img.stone.map_4.stone_1
                    else
                        ImagePath.toString Pages.images.img.stone.map_4.stone_1
                else if model.state == Three then
                    if pos.x2-pos.x1 >= pos.y2-pos.y1 then
                        ImagePath.toString Pages.images.img.stone.map_5.stone_1
                    else
                        ImagePath.toString Pages.images.img.stone.map_5.stone_2
                else
                    ImagePath.toString Pages.images.img.stone.map_1.stone_1
    in
...
renderBackground model=
    ...
    if model.state == LOGO then
        renderImage (ImagePath.toString Pages.images.img.background.background) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) [opacity (String.fromFloat (0))]
    else if model.state == Model.Start then 
        renderImage (ImagePath.toString Pages.images.img.page.start) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) [opacity (String.fromFloat (0.7+(abs(model.player.effecttimeHalf)/2000)))]
    else if model.state == End then
        renderImage (ImagePath.toString Pages.images.img.page.exit) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) [opacity (String.fromFloat (0.7+(abs(model.player.effecttimeHalf)/2000)))]
    else if model.state == Model.About then
        renderImage (ImagePath.toString Pages.images.img.page.about) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) [opacity "1"]
    else if model.state == Loading then
        renderImage (ImagePath.toString Pages.images.img.page.load_1) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) [opacity "1"]
        ...
        if model.player.pos.y1 >= 3200 || (model.player.pos.y2 <= 800 && model.player.pos.x1 >= 3200) then
            if model.player.pos.x1 <= 1600  then
                renderImage (ImagePath.toString Pages.images.img.background.background1_1) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) []
            else if model.player.pos.y2 <= 800 && model.player.pos.x1 >= 3200 then
                renderImage (ImagePath.toString Pages.images.img.background.background1_4) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) []
            else
                renderImage (ImagePath.toString Pages.images.img.background.background1_2) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) []
        else 
            renderImage (ImagePath.toString Pages.images.img.background.background1_3) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) []
    else if model.state == DiscoverI then
        if model.player.pos.y1 >= 3200 then
            renderImage (ImagePath.toString Pages.images.img.background.background2_1) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) []
        else
           renderImage (ImagePath.toString Pages.images.img.background.background2_2) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) []
    else if model.state == Two then
        renderImage (ImagePath.toString Pages.images.img.background.background3_1) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) []
    else if model.state == DiscoverII then
        if model.player.pos.y1 >=3200 then
            renderImage (ImagePath.toString Pages.images.img.background.background4_1) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) []
        else 
            renderImage (ImagePath.toString Pages.images.img.background.background4_2) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) []
    else if model.state == Three then
        renderImage (ImagePath.toString Pages.images.img.background.background5_1) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) []
    else
        Svg.image [][]
...
renderStoryBackground model = 
    let
        prefix = (Debug.log "cG1_4" (ImagePath.toString Pages.images.img.cG.cG1_4)) |> String.dropRight 9
        --prefix = "img/CG/" 
    in
    case model.state of
        Story1_5 ->
...
renderLOGO model = 
    renderImage (ImagePath.toString Pages.images.img.page.lOGO) (Pos 0 viewAttrs.size.x 0 viewAttrs.size.y) [opacity (String.fromFloat (1-(model.cgtime/1000-2.5)^4/40))]